...
def extract_observations(g: Graph):
    """
    Extracts observations where the ID is embedded in the Subject URI.
    """
    observations = []
    
    # Loop through all sosa:Observation subjects
    for obs in g.subjects(RDF.type, SOSA.Observation):
        
        # --- NEW LOGIC START ---
        # 1. Convert the Subject URI to a string so we can split it
        obs_uri = str(obs)
        
        # 2. Extract the ID from the URI
        # URI Format: http://example.com/observations/{id}/{timestamp}
        # Splitting by '/' creates a list, and we take the 2nd to last item
        try:
            id_val = obs_uri.split('/')[-2]
        except IndexError:
            print(f"Skipping malformed URI: {obs_uri}")
            continue
        # --- NEW LOGIC END ---

        # 3. Get the standard properties
        ...
        try:
            ...
            if not isinstance(time_val, datetime):
                # Fix for ISO strings that might have Z or T
...
def divide_data(observations):
    ...
    for (year, month, day), daily_obs in grouped.items():
        ...
        for obs, id_, result_value, property_, time_,runoff_val in daily_obs:
            ...
            # CHANGED: Register the member using the original Subject URI
            # Old code: metadata_graph.add((eventstream_uri, TREE.member, URIRef(f"{eventstream_uri}{id_}")))
            metadata_graph.add((eventstream_uri, TREE.member, obs))

            # Add the data triples
            g_snip.add((obs, RDF.type, SOSA.Observation))
            
            # REMOVED: g_snip.add((obs, EX.id, ...)) 
            # (Only add this back if you really want an explicit ID property in the output)
